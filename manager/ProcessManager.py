...
## Copyright (c) 2017, The Hongbao Project (www.hongbaoblockchain.org)
'''
Process managers for hongbaod, hongbao-wallet-cli and hongbao-wallet-rpc
'''
...
class HongbaodManager(ProcessManager):
    def __init__(self, resources_path, log_level=0, block_sync_size=10):
        proc_args = u'%s/bin/hongbaod --log-level %d --block-sync-size %d' % (resources_path, log_level, block_sync_size)
        ProcessManager.__init__(self, proc_args, "hongbaod")
        self.synced = Event()
        self.stopped = Event()
...
class WalletCliManager(ProcessManager):
    ...
    def __init__(self, resources_path, wallet_file_path, wallet_log_path, restore_wallet=False):
        if not restore_wallet:
            wallet_args = u'%s/bin/hongbao-wallet-cli --generate-new-wallet=%s --log-file=%s' \
                                                % (resources_path, wallet_file_path, wallet_log_path)
        else:
            wallet_args = u'%s/bin/hongbao-wallet-cli --log-file=%s --daemon-port 19735 --restore-deterministic-wallet' \
                                                % (resources_path, wallet_log_path)
        ProcessManager.__init__(self, wallet_args, "hongbao-wallet-cli")
        self.ready = Event()
        self.last_error = ""
...
class WalletRPCManager(ProcessManager):
    def __init__(self, resources_path, wallet_file_path, wallet_password, app, log_level=2):
        ...
        wallet_log_path = os.path.join(os.path.dirname(wallet_file_path), "hongbao-wallet-rpc.log")
        wallet_rpc_args = u'%s/bin/hongbao-wallet-rpc --wallet-file %s --log-file %s --rpc-bind-port 19736 --user-agent %s --log-level %d' \
                                            % (resources_path, wallet_file_path, wallet_log_path, self.user_agent, log_level)
                                                                                
        ProcessManager.__init__(self, wallet_rpc_args, "hongbao-wallet-rpc")
        sleep(0.2)
