...
from manager.ProcessManager import HongbaodManager, WalletRPCManager
...
class BaseWebUI(QMainWindow):
    def __init__(self, html, app, hub, window_size, debug=False):
        ...
        self.icon = self._getQIcon('hongbao_icon_64.png')
...
class MainWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        ...
        self.hongbaod_daemon_manager = None
...
#         self.view.load(qt_core.QUrl(self.url))
    ...
    def start_deamon(self):
        #start hongbaod daemon
        self.hongbaod_daemon_manager = HongbaodManager(self.app.property("ResPath"), 
                                            self.app_settings.settings['daemon']['log_level'], 
                                            self.app_settings.settings['daemon']['block_sync_size'])
        
        self.hongbaod_daemon_manager.start()
    ...
    def run_wallet_rpc(self, wallet_password, log_level=0):
        ...
        while True:
            ...
            hongbaod_info = self.daemon_rpc_request.get_info()
            if hongbaod_info['status'] == "OK":
    ...
    def _update_daemon_status(self):
        ...
        if hongbaod_info['status'] == "OK":
            status = "Connected"
            self.current_height = int(hongbaod_info['height'])
            target_height = int(hongbaod_info['target_height'])
...
#             print(json.dumps(wallet_info, indent=4))
    ...
    def about(self):
        QMessageBox.about(self, "About", \
            u"%s <br><br>Copyright© 2017 - Hongbao Projects (www.hongbaoblockchain.org)" % self.agent)
    ...
    def _handleAboutToQuit(self):
        ...
        if self.wallet_rpc_manager is not None:
            self.wallet_rpc_manager.stop()
        if self.hongbaod_daemon_manager is not None:
            self.hongbaod_daemon_manager.stop()
